...
int setupGeometry(int &numVertices)
{
	numVertices = 500;   // Quantidade de pontos para formar a espiral (define a suavidade)
    const float PI = 3.14159265359f;
	const float numRotations = 5.0f; // Quantas voltas a espiral dará
	const float maxRadius = 0.9f;    // O raio final da espiral

	vector<GLfloat> vertices;

	for (int i = 0; i < numVertices; i++)
	{
		// A proporção 'ratio' vai de 0.0 a 1.0 ao longo do loop
		float ratio = float(i) / float(numVertices);

		// O ângulo aumenta linearmente com a proporção, até o total de rotações
		float angle = ratio * numRotations * 2.0f * PI;

		// O raio também aumenta linearmente com a proporção
