...
int setupGeometry(int &numVertices)
{
	...
	// 2. Vértices do Perímetro
	for (int i = 0; i < numPoints; i++)
	{
		float angle = (2.0f * PI * float(i) / float(numPoints)) - (PI / 2.0f);
	...
	// 3. Adiciona o primeiro ponto do perímetro novamente no final da lista
	// para fechar o "leque" do GL_TRIANGLE_FAN.
	// O primeiro ponto do perímetro está nos índices 3, 4 e 5 (após o vértice central).
	vertices.push_back(vertices[3]);
	vertices.push_back(vertices[4]);
	vertices.push_back(vertices[5]);

	// A contagem de vértices agora será 1 (central) + 10 (perímetro) + 1 (fechamento) = 12
